...
class AdvancedBookParser {
  ...
  List<ChapterElement> _parseHtmlToElements(
    ...
    for (final match in pMatches) {
      ...
      // Verificăm dacă paragraful conține imagini
      // Folosim două regex-uri separate pentru " și '
      final imgRegexDouble = RegExp(r'<img[^>]+src="([^"]+)"[^>]*>');
      final imgRegexSingle = RegExp(r"<img[^>]+src='([^']+)'[^>]*>");

      final imgMatchesDouble = imgRegexDouble.allMatches(paragraphHtml);
      final imgMatchesSingle = imgRegexSingle.allMatches(paragraphHtml);
      final allImgMatches = [...imgMatchesDouble, ...imgMatchesSingle];
  ...
  Uint8List? _extractImageFromEpub(String imgSrc, epubx.EpubBook epubBook) {
    try {
      // Curățăm path-ul (eliminăm ../ și ../)
      String cleanSrc = imgSrc.replaceAll('../', '');

      // Căutăm în Images
      if (epubBook.Content?.Images != null) {
        for (final image in epubBook.Content!.Images!.values) {
          if (image.FileName?.endsWith(cleanSrc) ?? false) {
            // Convertim List<int>? la Uint8List?
